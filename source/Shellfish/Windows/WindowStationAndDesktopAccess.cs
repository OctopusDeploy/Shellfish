...
#pragma warning restore PC001 // API not supported on all platforms
        ...
        static NonReleasingSafeHandle GetProcessWindowStation()
        {
            var handle = NativeMethods.GetProcessWindowStation();
            if (handle.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(), "Failed to get a handle to the current window station for this process");
            return handle;
        }

        static NonReleasingSafeHandle GetThreadDesktop()
        {
            var handle = NativeMethods.GetThreadDesktop(NativeMethods.GetCurrentThreadId());
            if (handle.IsInvalid) throw new Win32Exception(Marshal.GetLastWin32Error(), "Failed to the a handle to the desktop for the current thread");
            return handle;
        }
        ...
        static class NativeMethods
        {
            ...
            public static extern NonReleasingSafeHandle GetProcessWindowStation();

            [DllImport("user32.dll", SetLastError = true)]
            public static extern NonReleasingSafeHandle GetThreadDesktop(int dwThreadId);
